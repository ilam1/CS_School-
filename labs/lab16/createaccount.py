...
print """<html>
 <head>
  <title>Lab 16 Create Account</title>
 </head>
<body>
...
#print form.getvalue("username") in text
if "submit" in form:
#    print "Entered cond statement"
    if "username" in form and "password" in form:
        ...
        for each in form.getvalue("username")+form.getvalue("password"):
            if each in '''!@#$%^&*()_+`-={}|[]\:"<>'?,./;''':
                ...
                error = True
        if error == False and '\n' + form.getvalue("username") + ',' not in text:
            ...
            f.close()
            print "Success!"
    if "username" in form and "password" in form and '\n' + form.getvalue("username") + "," in text:
